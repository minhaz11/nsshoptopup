...
class GatewayController extends Controller
{
    public function index()
    {
        ...
        return view('admin.gateway.list', compact('title', 'empty_message', 'gateways'));
    }

    public function edit($alias)
    {
        $gateway = Gateway::automatic()->with('currencies')->where('alias', $alias)->firstOrFail();
        $title = 'Update Gateway : ' . $gateway->name;
        ...
        return view('admin.gateway.edit', compact('title', 'gateway', 'supportedCurrencies', 'parameters', 'hasCurrencies', 'currency_idx', 'global_parameters'));
    }
    ...
    public function update(Request $request, $code)
    {
        ...
        if ($request->hasFile('image')) {
            try {
                $filename = imageUpload($request->image, $path, '200x200', $filename);
            } catch (\Exception $exp) {
            ...
            foreach ($request->currency as $key => $currency) {
                ...
                if ($request->hasFile($uploaded_image)) {
                    try {
                        ...
                        $filename = imageUpload($request->file($uploaded_image), $path, '200x200');
                    } catch (\Exception $exp) {
    ...
    public function gatewayValidator(Request $request)
    {
        $validation_rule = [
            ...
            'image' => 'nullable|image|mimes:jpg,jpeg,png,PNG,JPG',
        ];
    ...
    public function gatewayCurrencyValidator(Request $request, Gateway $gateway)
    {
        ...
        if ($request->has('currency')) {
            foreach ($request->currency as $key => $currency) {
                ...
                $validation_rule['currency.' . $key . '.image']          = ['nullable', 'image','mimes:png,jpg,jpeg,JPG,PNG'];
